...
def predict_aux(img_tensor, allFolds_models, cfg, aux_tasks, device):
    """
    Run the auxiliary binary heads on one image.

    Returns:
        dict {task: probability in [0, 1]} in the original label meaning,
        applying flipping for tasks listed in binary_aux_flip_targets (only
        for weighted-BCE loss models, not used for standard BCE loss).
    """
    probs = {}
    flip_targets = set(cfg.get("binary_aux_flip_targets", []))
    x = img_tensor.to(device)
...
def get_cam_overlap(cam_rsz, bbox, img_pil):
    ...
    Returns:
        scalar overlap score in [0,1] (or None if no bbox).
    """
    if bbox is None:
        return None
    W, H = img_pil.size
...
def generate_feedback_user(
    ...
    for task in aux_tasks:
        ...
        if prob is None or policy is None:
            continue
        if policy.get("display") == "exclude":
            continue

        thr = policy.get("threshold")
        lo = policy.get("uncertain_low")
        hi = policy.get("uncertain_high")

        # Uncertain band only if both bounds are defined.
        uncertain = (
            lo is not None
            and hi is not None
            and prob >= lo
        ...
        prefix = "[Supplementary] " if policy.get("display") == "supplementary" else ""

        if task == "Face":
            if uncertain:
                msg = "The model is uncertain whether the pet's face is clearly visible."
            elif thr is not None and prob >= thr:
            ...
            # Blur is currently excluded in label_policy, but we keep logic for completeness.
            if uncertain:
            ...
            # For other heads, you may add custom text later.
            continue
...
def generate_output_evaluation(
    ...
    """
    Evaluation view (not for user-facing feedback):
    ...
    for task in aux_tasks:
        ...
        policy = label_policy.get(task, {})
        thr = policy.get("threshold")
        display_mode = policy.get("display")

        # Compute binary prediction only for heads with a valid threshold and not excluded.
        if np.isnan(prob) or thr is None or display_mode == "exclude":
            pred_bin = None
        else:
            pred_bin = int(prob >= thr)

        t = true_aux.get(task, None)
        if t is not None and pred_bin is not None:
            ok = "✓" if int(t) == pred_bin else "✗"
            lines.append(
                f"  {task:<12} true={int(t)}  prob={prob:.3f}  pred={pred_bin}  {ok}"
            )
        else:
            lines.append(
                f"  {task:<12} true={t if t is not None else 'N/A'}  "
                f"prob={prob:.3f}  pred={pred_bin}"
            )
...
def visualise(
    ...
    ax4.axis("off")
    head = "Evaluation Output [Analysis Only, not for user]"
    if label:
        head += f" — {label}"
    ax4.set_title(head, fontsize=10, fontweight="bold", color="#888888")
    ax4.text(
        0.01,
        0.90,
        "\n".join(eval_lines),
...
# ---------------------------------------------------------------------
# Batch Runner (over a list of images)
# ---------------------------------------------------------------------
...
def run_batch(
    ...
    for img_id in image_ids:
        ...
        paw_score = predict_pawpularity(img_tensor, allFolds_models, device, fold=0)
        aux_probs = predict_aux(img_tensor, allFolds_models, cfg, aux_tasks, device)
        cam, cam_rsz, cam_ov = get_gradcam(
            img_tensor,
            img_pil,
            allFolds_models,
            device,
        ...
        # Decide save path; None => only show inline
        fig_path = os.path.join(save_dir, f"{img_id}_feedback.png") if save_dir else None

        # ALWAYS show the figure in the notebook (visualise calls plt.show())
        visualise(
            img_pil,
            cam_ov,
            user_fb,
        ...
        for task in aux_tasks:
            ...
            row[f"{task.lower()}_pred"] = pred_bin
